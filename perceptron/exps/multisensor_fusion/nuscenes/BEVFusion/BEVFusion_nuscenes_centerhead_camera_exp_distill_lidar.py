...
import numpy as np
import copy
from perceptron.exps.multisensor_fusion.nuscenes._base_.base_nuscenes_cfg import _POINT_CLOUD_RANGE, _VOXEL_SIZE, _GRID_SIZE, _OUT_SIZE_FACTOR
...
def calculate_box_mask_gaussian(preds_shape, target, pc_range, voxel_size, out_size_scale):
    B = preds_shape[0]
    C = preds_shape[1]
    H = preds_shape[2]
    W = preds_shape[3]
    ...
    for i in range(B):
        for j in range(len(target[i])):
            ...
            w, h = target[i][j][3]/(voxel_size[0]*out_size_scale), target[i][j][4]/(voxel_size[1]*out_size_scale)
            radius = gaussian_radius((w, h))
            radius = max(0, int(radius))

            center_heatmap = [int((target[i][j][0]-pc_range[0])/(voxel_size[0]*out_size_scale)),int((target[i][j][1]-pc_range[1])/(voxel_size[1]*out_size_scale))]
...
def FeatureDistillLoss(feature_lidar, feature_fuse, gt_boxes_bev_coords, gt_boxes_indices):
    h, w = feature_lidar.shape[-2:]
    gt_boxes_bev_center = torch.mean(gt_boxes_bev_coords,dim=2).unsqueeze(2)
    gt_boxes_bev_edge_1 = torch.mean(gt_boxes_bev_coords[:,:,[0,1],:], dim=2).unsqueeze(2)
    gt_boxes_bev_edge_2 = torch.mean(gt_boxes_bev_coords[:,:,[1,2],:], dim=2).unsqueeze(2)
    gt_boxes_bev_edge_3 = torch.mean(gt_boxes_bev_coords[:,:,[2,3],:], dim=2).unsqueeze(2)
    gt_boxes_bev_edge_4 = torch.mean(gt_boxes_bev_coords[:,:,[0,3],:], dim=2).unsqueeze(2)
    gt_boxes_bev_all = torch.cat((gt_boxes_bev_coords,gt_boxes_bev_center, gt_boxes_bev_edge_1,gt_boxes_bev_edge_2,gt_boxes_bev_edge_3,gt_boxes_bev_edge_4),dim=2)
    ...
    feature_lidar_sample = torch.nn.functional.grid_sample(feature_lidar, gt_boxes_bev_all)
    feature_lidar_sample = feature_lidar_sample.permute(0,2,3,1)
    feature_fuse_sample = torch.nn.functional.grid_sample(feature_fuse, gt_boxes_bev_all)
    feature_fuse_sample = feature_fuse_sample.permute(0,2,3,1)
    criterion = nn.L1Loss(reduce=False)
    loss_feature_distill = criterion(feature_lidar_sample[gt_boxes_indices], feature_fuse_sample[gt_boxes_indices])
...
def BEVDistillLoss(bev_lidar, bev_fuse, gt_boxes_bev_coords, gt_boxes_indices):
    ...
    weight = gt_boxes_indices.float().sum()
    weight = reduce_mean(weight)
    gt_boxes_sample_lidar_feature = feature_lidar_sample.contiguous().view(-1, feature_lidar_sample.shape[-2], feature_lidar_sample.shape[-1])
    gt_boxes_sample_fuse_feature = feature_fuse_sample.contiguous().view(-1, feature_fuse_sample.shape[-2], feature_fuse_sample.shape[-1])
    gt_boxes_sample_lidar_feature = gt_boxes_sample_lidar_feature / (torch.norm(gt_boxes_sample_lidar_feature, dim=-1, keepdim=True)+1e-4)
    gt_boxes_sample_fuse_feature =  gt_boxes_sample_fuse_feature / (torch.norm(gt_boxes_sample_fuse_feature, dim=-1, keepdim=True)+1e-4)
    gt_boxes_lidar_rel = torch.bmm(gt_boxes_sample_lidar_feature, torch.transpose(gt_boxes_sample_lidar_feature,1,2))
    gt_boxes_fuse_rel = torch.bmm(gt_boxes_sample_fuse_feature, torch.transpose(gt_boxes_sample_fuse_feature,1,2))
    gt_boxes_lidar_rel = gt_boxes_lidar_rel.contiguous().view(gt_boxes_bev_coords.shape[0],gt_boxes_bev_coords.shape[1],gt_boxes_lidar_rel.shape[-2],gt_boxes_lidar_rel.shape[-1])
    gt_boxes_fuse_rel = gt_boxes_fuse_rel.contiguous().view(gt_boxes_bev_coords.shape[0],gt_boxes_bev_coords.shape[1],gt_boxes_fuse_rel.shape[-2],gt_boxes_fuse_rel.shape[-1])
    loss_rel = criterion(gt_boxes_lidar_rel[gt_boxes_indices], gt_boxes_fuse_rel[gt_boxes_indices])
...
def ResponseDistillLoss(resp_lidar, resp_fuse, gt_boxes, pc_range, voxel_size, out_size_scale):
    ...
    for task_id, task_out in enumerate(resp_lidar):
        ...
        reg_lidar.append(torch.cat([task_out["reg"], task_out["height"], task_out["dim"], task_out["rot"], task_out["vel"], task_out["iou"]],dim=1))
        reg_fuse.append(torch.cat([resp_fuse[task_id]["reg"], resp_fuse[task_id]["height"],resp_fuse[task_id]["dim"],resp_fuse[task_id]["rot"],resp_fuse[task_id]["vel"],resp_fuse[task_id]["iou"]],dim=1))
    ...
    gaussian_mask = calculate_box_mask_gaussian(reg_lidar.shape, gt_boxes.cpu().detach().numpy(), pc_range, voxel_size, out_size_scale)
...
class Exp(BaseExp):
    def __init__(self, batch_size_per_device=4, total_devices=1, max_epoch=20, **kwargs):
        ...
        checkpoint = torch.load("perceptron/exps/multisensor_fusion/nuscenes/BEVFusion/tmp/lidar_model.pth", map_location=torch.device("cpu"))
    ...
    def training_step(self, batch):
        ...
        ret_dict, tf_dict, feature_lidar, bev_lidar, resp_lidar, _ = self.model(points, imgs, metas, gt_boxes)
        self.teacher_model.load_state_dict(self.checkpoint_state_dict, strict=False)
        feature_fuse,  bev_fuse, resp_fuse = self.teacher_model(points, imgs, metas, gt_boxes, return_feature=True)
        ...
        gt_boxes_bev_coords[:,:,:,0] = (gt_boxes_bev_coords[:,:,:,0]-_POINT_CLOUD_RANGE[0])/(_VOXEL_SIZE[0]*_OUT_SIZE_FACTOR)
        gt_boxes_bev_coords[:,:,:,1] = (gt_boxes_bev_coords[:,:,:,1]-_POINT_CLOUD_RANGE[1])/(_VOXEL_SIZE[1]*_OUT_SIZE_FACTOR)
        loss_feature = FeatureDistillLoss(feature_lidar, feature_fuse, gt_boxes_bev_coords, gt_boxes_indice)
        loss_bev_rel = BEVDistillLoss(bev_lidar, bev_fuse, gt_boxes_bev_coords, gt_boxes_indice)
        loss_resp_cls, loss_resp_reg = ResponseDistillLoss(resp_lidar, resp_fuse, gt_boxes, _POINT_CLOUD_RANGE, _VOXEL_SIZE, _OUT_SIZE_FACTOR)
        tf_dict.update({'loss_feature':loss_feature, 'loss_bev_rel':loss_bev_rel, 'loss_resp_cls':loss_resp_cls, 'loss_resp_reg':loss_resp_reg})
        loss = ret_dict["loss"].mean()+100*(loss_feature)+40*loss_bev_rel+10*(loss_resp_cls+loss_resp_reg)
        end_time = time.time()
        return loss, tf_dict
