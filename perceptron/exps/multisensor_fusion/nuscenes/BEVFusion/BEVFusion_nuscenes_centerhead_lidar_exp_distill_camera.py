...
class Exp(BaseExp):
    def __init__(self, batch_size_per_device=4, total_devices=1, max_epoch=20, **kwargs):
        ...
        checkpoint = torch.load("perceptron/exps/multisensor_fusion/nuscenes/BEVFusion/tmp/camera_model.pth", map_location=torch.device("cpu"))
    ...
    def training_step(self, batch):
        ...
        loss = ret_dict["loss"].mean()+10*(loss_feature)+5*loss_bev_rel+1*(loss_resp_cls+loss_resp_reg)
        end_time = time.time()
        return loss, tf_dict
