...
class NuscenesMultiModalDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if item_info.get("ann_infos", None) is not None:
            for i in range(len(item_info["ann_infos"])):
                item_info["ann_infos"][i]["gt_name"] = _map_name_from_general_to_detection[
    ...
    def get_mapping_names(self, gt_names, mapping_dict=_map_name_from_general_to_detection):
        return np.array(list(map(lambda x: mapping_dict[x], gt_names)))

    def _get_images(self, idx, keys):
        result = {}
        for k in keys:
            img_file = os.path.join('/data/dataset/', self.infos[idx]['cam_infos'][k]['filename'])
    ...
    def _load_camera_sweeps(self, item_info: Dict, item: Dict) -> None:
        ...
        for sw_idx in self.cam_sweeps_idx:
            ...
            for cam in self.img_key_list:
                ...
                while cam_sw_idx >= 0 and cam not in item_info["cam_sweeps"][cam_sw_idx]:
                    cam_sw_idx -= 1
                if cam_sw_idx >= 0:
                    sw_img_file = os.path.join('/data/dataset/', item_info["cam_sweeps"][cam_sw_idx][cam]["filename"])
    ...
    def _get_point_cloud(self, idx, keys):
        ...
        result = {}
        for k in keys:
            point_file = os.path.join('/data/dataset/', self.infos[idx]['lidar_infos'][k]['filename'])
            point_cloud = np.fromfile(point_file,dtype=np.float32,count=-1).reshape(-1,5)
    ...
    def _load_lidar_sweeps(self, item_info: Dict, item: Dict) -> None:
        ...
        for sw_idx in self.lidar_sweeps_idx:
            ...
            for lidar in self.lidar_key_list:
                ...
                if lidar_sw_idx >= 0:
                    sw_point_file = os.path.join('/data/dataset/',item_info["lidar_sweeps"][lidar_sw_idx][lidar]["filename"])
                    point_cloud = np.fromfile(sw_point_file,dtype=np.float32,count=-1).reshape(-1,5)
                    sw_lidar[lidar] = (
                        point_cloud[:, :load_dim].copy()
                    )
                    sw_lidar_infos[lidar] = item_info["lidar_sweeps"][lidar_sw_idx][lidar]
                else:
                    sw_lidar[lidar] = copy.deepcopy(item["points"][lidar])
                    sw_lidar_infos[lidar] = copy.deepcopy(item_info["lidar_infos"][lidar])
