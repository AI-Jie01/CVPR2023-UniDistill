...
class NuscenesMultiModalData(NuscenesMultiModalDataset):
    ...
    def get_det_augmentor(self, aug_cfg):
        ...
        if aug_cfg.get("ida_aug_cfg", None):
            augmentor_list.append(
                transforms3d.ImageAffineTransformation(**aug_cfg["ida_aug_cfg"], is_train=self.is_train)
            )
        if self.is_train:
            if aug_cfg.get("gt_sampling_cfg", None):
                augmentor_list.append(transforms3d.GTSampling(**aug_cfg["gt_sampling_cfg"]))
            if aug_cfg.get("bda_aug_cfg", None):
                augmentor_list.append(transforms3d.BevAffineTransformation(**aug_cfg["bda_aug_cfg"]))
        augmentor_list.append(transforms3d.ObjectRangeFilter(aug_cfg["point_cloud_range"]))
    ...
    def _get_sample_indices(self):
        ...
        for idx, info in enumerate(self.infos):
            ...
            gt_names = set([map_name_from_general_to_detection[gt_name] for gt_name in gt_names])
    ...
    def _generate_data_dict(self, org_data: Dict) -> Dict:
        ...
        if self.is_train:
            mask = [
                ...
                and (org_data["info"]["num_lidar_pts"][i] + org_data["info"]["num_radar_pts"][i]) > 0
                for i in range(len(org_data["info"]["gt_boxes"]))
            ]
            data_dict["gt_boxes"] = org_data["info"]["gt_boxes"][mask]
            data_dict["gt_labels"] = np.array([self.classes.index(i) for i in org_data["info"]["gt_names"][mask]])
        else:
        ...
        data_dict["info"]["ego_to_global"] = np.linalg.inv(org_data["info"]["car_from_global"])

        if self.with_lidar:
            data_dict["with_lidar"] = True
            data_dict["info"]["lidar_to_ego"] = np.linalg.inv(org_data["info"]["ref_from_car"])
            data_dict["points"] = org_data["points"]["LIDAR_TOP"]
            if "sweep_points" in org_data:
                data_dict["sweep_points"] = [sweep_pts["LIDAR_TOP"] for sweep_pts in org_data["sweep_points"]]
                new_sweep_infos = []
                for sweep_info in org_data["info"]["sweep_lidar_infos"]:
                    new_sweep_info = dict(
                        sweep_lidar_to_ego=np.linalg.inv(sweep_info["LIDAR_TOP"]["car_from_global"]),
    ...
    def format_processor(self, data_dict, data):
        ...
        if self.with_camera:
            ...
            for cam_name in self.img_key_list:
                ...
                sensor2ego_mat[:3, :3] = torch.Tensor(
                    Quaternion(data["info"]["sensor2ego_rotations"][cam_name]).rotation_matrix
                )
                sensor2ego_mat[:3, 3] = torch.Tensor(data["info"]["sensor2ego_translations"][cam_name])
                sensor2ego_mat = torch.Tensor(data["info"]["ref_from_car"]).to(sensor2ego_mat.dtype) @ sensor2ego_mat
                sensor2ego_mats.append(sensor2ego_mat)
                intrin_mat = torch.eye(4)
                intrin_mat[:3, :3] = torch.Tensor(
                    data["info"]["cam_infos"][cam_name]["calibrated_sensor"]["camera_intrinsic"]
                )
                intrin_mats.append(intrin_mat)
                ida_mats.append(torch.from_numpy(data_dict["ida_mat"][cam_name]))
                sensor2sensor_mats.append(torch.eye(4, 4))
            ret_data["imgs"] = torch.stack(imgs).unsqueeze(0)  # TODO: modify after finish CollectCameraSweeps
        ...
        ret_data["gt_boxes"][np.isnan(ret_data["gt_boxes"])] = 0  # replace nan velocity of cones and barriers
...
def collate_fn(data, is_return_depth=False, with_points=True):
    ...
    mats_keys = ["sensor2ego_mats", "intrin_mats", "ida_mats", "sensor2sensor_mats", "bda_mat"]
    if "mats_dict" in data[0]:
        ...
        for key in mats_keys:
            if key in data[0]["mats_dict"]:
                ...
                batch_collection["mats_dict"][key] = torch.stack(data_list).to(torch.float32)
