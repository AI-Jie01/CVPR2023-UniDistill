...
from .functional import  center_to_corner_box3d, img_transform, bev_transform
...
class ImageAffineTransformation(torch.nn.Module):
    ...
    def sample_augs(self):
        ...
        if self.is_train:
            ...
            crop_h = int((1 - np.random.uniform(*self.aug_conf["bot_pct_lim"])) * newH) - fH
    ...
    def forward(self, data_dict):
        if data_dict.get("points", None) is not None:
            ...
            for swid, frame in enumerate(data_dict.pop("sweep_points")):
                sweep_ego_to_global = data_dict["info"]["sweep_lidar_infos"][swid]["sweep_lidar_to_ego"]
                ...
                if all_points.shape[-1] == 5:
                    frame[:, -1] = (
                        ...
                        - data_dict["info"]["sweep_lidar_infos"][swid]["sweep_lidar_timestamp"]
                    ) / 1e6
...
class BevAffineTransformation(torch.nn.Module):
    ...
    def forward(self, data_dict):
        ...
        gt_boxes = data_dict["gt_boxes"]
        gt_boxes, transform_mat = bev_transform(gt_boxes, rotate_bda, scale_bda, trans_bda, flip_dx, flip_dy)
        if data_dict.get("points", None) is not None:
