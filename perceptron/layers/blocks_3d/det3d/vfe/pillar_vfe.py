...
class PillarVFE(VFETemplate):
    ...
    def forward(self, voxel_features, voxel_coords, voxel_num_points, **kwargs):
        ...
        f_center[:, :, 0] = voxel_features[:, :, 0] - (
            voxel_coords[:, 3].to(voxel_features.dtype).unsqueeze(1) * self.voxel_x + self.x_offset
        )
        f_center[:, :, 1] = voxel_features[:, :, 1] - (
            voxel_coords[:, 2].to(voxel_features.dtype).unsqueeze(1) * self.voxel_y + self.y_offset
        )
        f_center[:, :, 2] = voxel_features[:, :, 2] - (
            voxel_coords[:, 1].to(voxel_features.dtype).unsqueeze(1) * self.voxel_z + self.z_offset
        )
