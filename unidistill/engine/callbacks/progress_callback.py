...
from unidistill.utils.log_utils import AvgMeter
...
class ProgressBar(MasterOnlyCallback):
    ...
    def setup(self, trainer: Trainer):
        self.epoch_bar = tqdm.tqdm(
            initial=0, total=trainer.exp.max_epoch, desc="[Epoch]", dynamic_ncols=True
        )
        self.step_bar = tqdm.tqdm(
            initial=0, desc="[Step]", dynamic_ncols=True, leave=False
        )
        if self.logger:
...
class TextMonitor(MasterOnlyCallback, LearningRateMonitor):
    ...
    def after_step(self, trainer: Trainer, step, data_dict, *args, **kwargs):
        ...
        ext_info = "".join(
            [f" {k}={v.window_avg :.4f}" for k, v in self.ext_dict.items()]
        )
                ...
                if ext_dict is not None:
                    for key, val in ext_dict.items():
                        self.tb_log.add_scalar(
                            f"train/{key}_{dataset_name}", val, accumulated_iter
                        )
        else:
            if kwargs["extra"] is not None:
                for key, val in kwargs["extra"].items():
